...
# Assignment: x
	addiu $a0, $zero, 10	# Load integer literal 10
	sw $a0, -4($fp)	# Store to local x
# Assignment: y
	addiu $a0, $zero, 20	# Load integer literal 20
...
# Assignment: z
	...
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	addiu $a0, $zero, 2	# Load integer literal 2
	lw $t1, 0($sp)	# Pop previous result to temp
	addi $sp, $sp, 4	# Deallocate stack space
...
# Assignment: globalFlag
	...
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	addiu $a0, $zero, 15	# Load integer literal 15
	lw $t1, 0($sp)	# Pop previous result to temp
	addi $sp, $sp, 4	# Deallocate stack space
...
# Prologue (MIPS ABI compliant)
	addiu $sp, $sp, -32	# Allocate frame (32 bytes)
	sw $ra, 28($sp)	# Save return address
	sw $fp, 24($sp)	# Save frame pointer
	addiu $fp, $sp, 32	# FP points to old SP
	
# Method body
# Local variable x at offset -24
# Local variable y at offset -28
# Local variable z at offset -32
# Local variable test at offset -36
# Local variable flag2 at offset -40
# Assignment: x
	addiu $a0, $zero, 10	# Load integer literal 10
	sw $a0, -24($fp)	# Store to local x
# Assignment: y
	addiu $a0, $zero, 20	# Load integer literal 20
	sw $a0, -28($fp)	# Store to local y
# Assignment: z
	lw $a0, -24($fp)	# Load local x to accumulator
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	lw $a0, -28($fp)	# Load local y to accumulator
	lw $t1, 0($sp)	# Pop previous result to temp
	addi $sp, $sp, 4	# Deallocate stack space
	add $a0, $t1, $a0	# Add: $t1 + $a0
	sw $a0, -32($fp)	# Store to local z
# Assignment: test
	lw $a0, -24($fp)	# Load local x to accumulator
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	...
	addi $sp, $sp, 4	# Deallocate stack space
	slt $a0, $t1, $a0	# Less than: $t1 < $a0
	sw $a0, -36($fp)	# Store to local test
# Assignment: flag2
	lw $a0, -24($fp)	# Load local x to accumulator
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	...
	xor $a0, $t1, $a0	# XOR for equality
	sltiu $a0, $a0, 1	# Set if equal (result is 0)
	sw $a0, -40($fp)	# Store to local flag2
	lw $a0, -36($fp)	# Load local test to accumulator
	beq $a0, $zero, if_end_1	# If condition false, skip body
# Assignment: z
	lw $a0, -32($fp)	# Load local z to accumulator
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	addiu $a0, $zero, 2	# Load integer literal 2
	lw $t1, 0($sp)	# Pop previous result to temp
...
# Assignment: globalFlag
	...
	xor $a0, $t1, $a0	# XOR for equality
	sltiu $a0, $a0, 1	# Set if equal (result is 0)
	beq $a0, $zero, and_false_1	# AND: short-circuit if left is false
	lw $a0, -28($fp)	# Load local y to accumulator
	addi $sp, $sp, -4	# Allocate stack space
	sw $a0, 0($sp)	# Push left operand
	addiu $a0, $zero, 15	# Load integer literal 15
...
main_exit:
	...
	addiu $sp, $sp, 32	# Deallocate frame
	jr $ra	# Return
